...
// Dynamic import guard — Leaflet uses window/document
let L: any = null;
...
function createVehicleIcon(vehicle: VehicleState, selected: boolean) {
...
export default function LiveMap({ vehicles, selectedId, onVehicleClick }: Props) {
  ...
  useEffect(() => {
    ...
    vehicles.forEach(vehicle => {
      ...
      const icon = createVehicleIcon(vehicle, vehicle.vehicleId === selectedId);

      if (markersRef.current[vehicle.vehicleId]) {
        // Update existing marker
        const marker = markersRef.current[vehicle.vehicleId];
        ...
        const marker = L.marker(latlng as any, { icon })
          .addTo(map)
          .on('click', () => onVehicleClick(vehicle.vehicleId));
      ...
      const popup = `
        <div style="font-family: 'JetBrains Mono', monospace; background: #0D1220; color: #E2E8F0; padding: 10px; border-radius: 6px; min-width: 160px;">
          ...
          ${vehicle.plateNumber ? `<div style="font-size: 11px; color: #64748B; margin-bottom: 4px;">${vehicle.plateNumber}</div>` : ''}
          <div style="font-size: 13px;">
            🏎 ${vehicle.speed ?? 0} km/h &nbsp; ↗ ${vehicle.heading?.toFixed(0) ?? '–'}°
          </div>
      ...
      markersRef.current[vehicle.vehicleId].bindPopup(popup, {
  ...
  useEffect(() => {
    ...
    if (vehicle?.lat && vehicle?.lng) {
      mapRef.current.flyTo([vehicle.lat, vehicle.lng], 15, { animate: true, duration: 0.8 });
    }
  }, [selectedId]);
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        .fleet-popup .leaflet-popup-content { margin: 0 !important; }
        .fleet-popup .leaflet-popup-tip { display: none; }
